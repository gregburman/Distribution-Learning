...
def create_network(input_shape, name):
	...
	unet, _, _ = mala.networks.unet(
		...
		num_fmaps=12,
		fmap_inc_factors=3,
		downsample_factors=[[2,2,2],[2,2,2],[2,2,2]]) # 2,2,2 etc or 3,3,3 (downsample the same amount in all dimensions  because toy data isotropic)
	...
	loss = tf.losses.mean_squared_error(
		...
		affs,
		affs_loss_weights)
	# loss = tf.losses.sigmoid_cross_entropy(gt_affs, affs, affs_loss_weights)
